...
namespace contoso.ado.PostgreSql
{
    ...
    public class AsyncTextExport<TOut>
    {
        readonly PostgresCommandContext _ctx;
        readonly Func<TextReader, Task<TOut>> _readDataAsync;

        internal AsyncTextExport(PostgresCommandContext cmdCtx, Func<TextReader, Task<TOut>> readDataAsync) { _ctx = cmdCtx; _readDataAsync = readDataAsync; }

        /// <summary>
        /// Invokes <see cref="M:Npgsql.NpgsqlConnection.BeginTextExport"/> on the <see cref="NpgsqlConnection"/> in context,
        /// <para>The underlying command text is of the format: "COPY tbl (col1, col2) TO STDIN".</para>
        /// </summary>
        public async Task<TOut> ReadBulkExportAsync()
        {
            ...
            try
            {
                using (var connection = _ctx.CreateNpgsqlConnection())
                {
                    ...
                    using TextReader reader = connection.BeginTextExport(_ctx.Command.CommandText);
