...
internal sealed class TarGzStreamToDirectory : UnzipAndExtractToDirectory
{
    readonly StreamToDirectory _io;

    internal TarGzStreamToDirectory(in StreamToDirectory ioPair)
        : base()
    {
        _io = ioPair;
    ...
    public override ExtractToDirectoryResult UnzipAndExtract()
    {
        ...
        try
        {
            // decompress the gzip stream to memory as tar
            using (MemoryStream unzippedStream = new MemoryStream(TarGzReader.UnzipAllocation))
            {
                DecompressToStreamResult unzipResult = GZipReader.CreateFor(_io.Input.Stream)
